...
impl ContractInteract {
    ...
    async fn add_guild_master_tiers(&mut self) {
        let tiers = MultiValueVec::from(vec![MultiValue2::from((BigUint::<StaticApi>::from(0u128), 0u64))]);
    ...
    async fn add_user_tiers(&mut self) {
        let tiers = MultiValueVec::from(vec![MultiValue2::from((0u64, 0u64))]);
